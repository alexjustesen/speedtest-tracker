    ...
    <div class="grid gap-4 sm:grid-cols-6 sm:gap-8">
        ...
        @isset($latestResult)
            <div class="text-sm font-semibold leading-6 text-center col-span-full sm:text-base">
                Latest result: <time title="{{ $latestResult->created_at->timezone(app(GeneralSettings::class)->display_timezone)->format(app(GeneralSettings::class)->datetime_format) }}" datetime="{{ $latestResult->created_at->timezone(app(GeneralSettings::class)->display_timezone) }}">{{ $latestResult->created_at->diffForHumans() }}</time>
            </div>
        @endisset
