<x-dashboard-layout title="Dashboard V2">
    <div x-data="dashboard()" x-init="init()" class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            {{-- Ping Metrics --}}
            <div x-show="pingStats" class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 sm:p-6">
                <h3 class="text-lg font-semibold mb-4">Ping / Latency</h3>

                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    {{-- Chart --}}
                    <div class="lg:col-span-2">
                        <canvas id="ping-chart" class="w-full" style="height: 300px;"></canvas>
                    </div>
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-1 gap-4">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                            <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Latest</div>
                            <div class="text-xl font-bold mt-1" x-text="pingStats ? formatPing(pingStats.latest) : '-'"></div>
                        </div>

                        {{-- Average --}}
                        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                            <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Average</div>
                            <div class="text-xl font-bold mt-1" x-text="pingStats ? formatPing(pingStats.average) : '-'"></div>
                        </div>

                        {{-- Lowest --}}
                        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                            <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Lowest</div>
                            <div class="text-xl font-bold mt-1" x-text="pingStats ? formatPing(pingStats.lowest) : '-'"></div>
                        </div>

                        {{-- Highest --}}
                        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                            <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Highest</div>
                            <div class="text-xl font-bold mt-1" x-text="pingStats ? formatPing(pingStats.highest) : '-'"></div>
                        </div>
                    </div>
                </div>
            </div>
