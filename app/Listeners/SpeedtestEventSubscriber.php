...
use App\Jobs\Notifications\Apprise\SendSpeedtestFailedNotification as AppriseFailed;
...
use App\Jobs\Notifications\Database\SendSpeedtestFailedNotification as DatabaseFailed;
...
use App\Jobs\Notifications\Mail\SendSpeedtestFailedNotification as MailFailed;
...
use App\Jobs\Notifications\Webhook\SendSpeedtestFailedNotification as WebhookFailed;
...
class SpeedtestEventSubscriber
{
    ...
    public function handleSpeedtestFailed(SpeedtestFailed $event): void
    {
        ...
        if ($notificationSettings->database_enabled) {
            if ($notificationSettings->database_on_speedtest_failed) {
                DatabaseFailed::dispatch($event->result);
            }
        }

        // Apprise notifications
        if ($notificationSettings->apprise_enabled) {
            if ($notificationSettings->apprise_on_speedtest_failed) {
                AppriseFailed::dispatch($event->result);
            }
        }

        // Webhook notifications
        if ($notificationSettings->webhook_enabled) {
            if ($notificationSettings->webhook_on_speedtest_failed) {
                WebhookFailed::dispatch($event->result);
            }
        }

        // Mail notifications
        if ($notificationSettings->mail_enabled) {
            if ($notificationSettings->mail_on_speedtest_failed) {
                MailFailed::dispatch($event->result);
            }
        }
    }
